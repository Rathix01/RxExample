...
const toListenersFor = R.curry((listener2, listener3, state) => {
	return Rx.Observable.zip(listener2, listener3);
}); 
...
const inputUrl4 = RxExampleActions.input4.mapTo('https://api.github.com/users');
...
Rx.Observable.merge(ajaxRequest1, ajaxRequest2, ajaxRequest3, ajaxRequest4).map(toStateWithKey("merge")).subscribe(toConsoleLog);

//log response when each request has 
Rx.Observable.zip(ajaxRequest1, ajaxRequest2, ajaxRequest3).map(toStateWithKey("zip")).subscribe(toConsoleLog);

//something a little more complex.
//when ajaxRequest4 is completed, ajaxRequest2 and ajaxRequest3 listeners are created.
ajaxRequest4.switchMap(toListenersFor(ajaxRequest2, ajaxRequest3)).map(toStateWithKey("conditional zip")).subscribe(toConsoleLog);